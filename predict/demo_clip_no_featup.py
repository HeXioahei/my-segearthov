...
def build_parser():
    parser = argparse.ArgumentParser(
        description='SegEarth-OV demo (no featup): baseline CLIP ViT-B/16 open-vocabulary segmentation'
    )
    parser.add_argument(
        '--img',
        type=str,
    ...
    parser.add_argument(
        '--out',
        type=str,
        default='seg_pred_clip_b16_no_featup.png',
        help='分割结果保存路径（默认：seg_pred_clip_b16_no_featup.png）'
    )
    return parser
...
def main():
    ...
    # 构建“原论文风格”的 baseline 模型：
    #   - clip_type='CLIP'
    #   - vit_type='ViT-B/16'
    #   - feature_up=False（显式关闭 SimFeatUp）
