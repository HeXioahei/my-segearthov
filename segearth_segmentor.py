...
class SegEarthSegmentation(BaseSegmentor):
    def __init__(self,
                 ...
                 prob_thd=0.0, # 概率阈值，即预测概率小于该阈值的像素被视为背景
                 logit_scale=50,
                 slide_stride=112,
                 slide_crop=224, # 滑动裁剪尺寸
                 cls_token_lambda=-0.3, # 类别token lambda
                 bg_idx=0, # 背景索引
                 feature_up=False,
                 feature_up_cfg=dict(
                     model_name='jbu_one',
                     model_path='/root/project/SegEarth-OV/simfeatup_dev/weights/xclip_jbu_one_million_aid.ckpt'),
                 use_clipself_georsclip_l14=False,
                 # GeoRSCLIP: keep text tower from RS5M (text_pretrained_path),
                 # override only visual tower weights from visual_pretrained_path.
        ...
        def _load_visual_only(clip_model: nn.Module, ckpt_path: str):
            if not ckpt_path:
                return
            ...
            if not isinstance(state_dict, dict):
                raise ValueError(f'Unsupported checkpoint format for visual_pretrained_path: {ckpt_path}')

            # strip common prefixes
            if state_dict and next(iter(state_dict.keys())).startswith('module.'):
                state_dict = {k[7:]: v for k, v in state_dict.items()}

            # if checkpoint is full CLIP, keep only visual.*
            if any(k.startswith('visual.') for k in state_dict.keys()):
                state_dict = {k[len('visual.'):]: v for k, v in state_dict.items() if k.startswith('visual.')}

            incompatible = clip_model.visual.load_state_dict(state_dict, strict=False)
            if getattr(incompatible, 'missing_keys', None) or getattr(incompatible, 'unexpected_keys', None):
                print(
                    f'[SegEarthSegmentation] Info: loaded visual weights from {ckpt_path} with '
        ...
        if clip_type == 'CLIP':
            ...
            if 'B' in vit_type:
                # GeoRSCLIP (ViT-B-32):
                # - keep text tower from RS5M (text_pretrained_path if provided)
                # - optionally override ONLY visual tower weights (visual_pretrained_path)
                text_ckpt = text_pretrained_path or 'checkpoint/RS5M_ViT-B-32.pt'
                self.net = create_model('ViT-B/32', pretrained=text_ckpt, precision='fp32')
                _load_visual_only(self.net, visual_pretrained_path)
            elif 'L' in vit_type:
                if use_clipself_georsclip_l14:
                    # 使用经过 CLIPSelf 训练的 GeoRSCLIP ViT-L-14 权重
                    self.net = create_model('ViT-L-14', pretrained='checkpoint/RS5M_ViT-L-14_CLIPSelf.pth', precision='fp16')
                else:
                    # 使用原始 GeoRSCLIP ViT-L-14 权重（未经过 CLIPSelf 训练）
        ...
        elif clip_type == 'ReoRSCLIP':
            # Backwards-compat alias; prefer using clip_type='GeoRSCLIP' + visual_pretrained_path/text_pretrained_path.
            if 'B' in vit_type:
                text_ckpt = text_pretrained_path or 'checkpoint/RS5M_ViT-B-32.pt'
                self.net = create_model('ViT-B/32', pretrained=text_ckpt, precision='fp16')
                # self.net = create_model('ViT-B/32', pretrained=text_ckpt, precision='fp32')
                _load_visual_only(self.net, visual_pretrained_path)
            else:
                raise ValueError(f'ReoRSCLIP currently supports only ViT-B-32, got vit_type={vit_type}')
        ...
        if feature_up:
            ...
            # 当前仓库中的三个 SimFeatUp 权重（clip/maskclip/xclip_jbu_*）均为 512 维特征设计，
            # 与 GeoRSCLIP ViT-L-14 的 768 维特征不匹配。
            # 为避免随机初始化导致的不稳定效果：
            #   - 若 feat_dim == 512，则正常加载 SimFeatUp 权重并启用 feature_up；
            #   - 若 feat_dim != 512，则关闭 feature_up，仅使用 backbone 特征。
            if self.feat_dim != 512:
                print(
                    f'[SegEarthSegmentation] Info: feature_up is disabled automatically '
                    f'because feat_dim={self.feat_dim} has no compatible SimFeatUp weights '
                    f'in simfeatup_dev/weights (all are for 512-d features).',
                    flush=True
                )
                self.feature_up = False
            else:
                # 尝试初始化 SimFeatUp，如果失败则自动关闭 feature_up
                try:
                    ...
                    print(
                        f'[SegEarthSegmentation] Warning: Failed to initialize SimFeatUp (feat_dim={self.feat_dim}): {e}. '
                        f'Disabling feature_up to avoid errors. '
                        f'This may be due to missing CUDA extensions or incompatible feature dimensions.',
                        flush=True
                    )
                    self.feature_up = False
